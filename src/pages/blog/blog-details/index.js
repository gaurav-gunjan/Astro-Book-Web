...
const BlogDetails = () => {
    ...
    useEffect(() => {
        ...
        dispatch(BlogActions.getRecentAstroblog({ page: 1, limit: 3 }));
    }, []);
            ...
            <section className='px-[80px] max-md:px-[20px] pt-5 pb-14'>
                <article className='flex gap-5'>
                    <section className='flex-1 flex flex-col justify-start gap-5'>
                        ...
                        <img src={api_urls + 'uploads/' + product?.image} className="w-full h-[500px] max-h-[550px] rounded-lg object-contain" />
                    ...
                    <div className='flex flex-col pl-5 border-l border-[#B0B0B0] basis-[25%]'>
                        ...
                        <div className='flex flex-col gap-3 py-8 text-[18px]'>
                ...
                <main className='flex flex-wrap gap-[2.5%] gap-y-[40px]'>
                    {recentAstroBlogData && recentAstroBlogData?.map((value, index) => (
                        <div
                            key={index} onClick={() => {
                                window.scrollTo({ top: 0, behavior: 'smooth' });
                                navigate('/blog/blog-details', { state: { astroBlogData: value } })
                            }}
                            className='relative flex flex-col border border-primary pb-4 rounded-[24.61px] lg:basis-[31.5%] max-lg:basis-[47.5%] max-lg:flex-grow max-md:basis-full cursor-pointer h-[287.69px]'>
